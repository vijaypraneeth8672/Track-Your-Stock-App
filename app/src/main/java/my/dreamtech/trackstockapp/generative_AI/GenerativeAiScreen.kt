...
@Composable
@Destination
fun AppContent(viewModel: GenerativeAiViewModel = hiltViewModel()){
    val  generativeAiState = viewModel.uiState.collectAsState()

    val coroutineScope = rememberCoroutineScope()
    val imageRequestBuilder = ImageRequest.Builder(LocalContext.current)
    val imageLoader = ImageLoader.Builder(LocalContext.current).build()

    GenerativeAiScreen(uiState = generativeAiState.value){
...
fun GenerativeAiScreen(
    uiState: GenerativeAiState = GenerativeAiState.Loading,
    ...
    Scaffold(
        ...
        Column(
            ...
            when (uiState) {
                is GenerativeAiState.Initial -> {}
                is GenerativeAiState.Loading -> {
                    Box(contentAlignment = Alignment.Center) {
                        CircularProgressIndicator()
                    }
                }

                is GenerativeAiState.Success -> {
                    Card(
                        modifier = Modifier
